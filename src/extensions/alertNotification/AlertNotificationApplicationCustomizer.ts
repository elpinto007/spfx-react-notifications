...
const LOG_SOURCE: string = 'AlertNotificationApplicationCustomizer';

export interface IAlertNotificationApplicationCustomizerProperties {
}

/** A Custom Action which can be run during execution of a Client Side Application */
export default class AlertNotificationApplicationCustomizer
  extends BaseApplicationCustomizer<IAlertNotificationApplicationCustomizerProperties> {
  ...
  private _render(): void {
    ...
    if (!AlertNotificationApplicationCustomizer._topPlaceholder) {
      // create a DOM element in the top placeholder for the application customizer
      // to render
      AlertNotificationApplicationCustomizer._topPlaceholder = this.context.placeholderProvider
    ...
    this
      ...
      .then((connectedHubSiteData: IHubSiteData): Promise<IAlert[]> => {
        if (connectedHubSiteData === null) {
          ...
          // error code.
          return Promise.reject(AlertNotificationApplicationCustomizer.NO_HUBSITE_DATA);
        }
      ...
      .then((upcomingAlerts: IAlert[]): void => {
        ...
        ReactDom.render(element, AlertNotificationApplicationCustomizer._topPlaceholder.domElement);
      })
      .catch((error: any): void => {
        if (error === AlertNotificationApplicationCustomizer.NO_HUBSITE_DATA) {
